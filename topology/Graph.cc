...
bool Graph::setVetexNum(int numVetex) {
    m_nVertexNum = numVetex;
    return 0;
}
void Graph::finishAddingVertex() {
    m_nVertexNum = m_vtVertices.size();
    m_nEdgeNum = m_mpEdgeCodeWeight.size();
}
bool Graph::addEdge(int fromnode, int tonode) {
    BaseVertex* start_vertex_pt = get_vertex(fromnode);
    BaseVertex* end_vertex_pt = get_vertex(tonode);
...
void Graph::_import_from_file(const string& input_file_name) {
    ...
    if (m_nVertexNum != m_vtVertices.size()) {
        cerr << "The number of nodes in the graph is " << m_vtVertices.size()
                << " instead of " << m_nVertexNum << endl;
        exit(1);
    }
...
void Graph::clear() {
    ...
    for (map<BaseVertex*, set<BaseVertex*>*>::const_iterator pos =
            m_mpFanoutVertices.begin(); pos != m_mpFanoutVertices.end();
            ++pos) {
        delete pos->second;
    }
...
set<BaseVertex*>* Graph::get_vertex_set_pt(BaseVertex* vertex_,
...
double Graph::get_edge_weight(const BaseVertex* source,
