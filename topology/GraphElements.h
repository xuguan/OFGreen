...
public:
    ...
    int SharedLinks(BasePath * otherPath) {
        int count = 0;
        if (m_nLength < 2 || otherPath->length() < 2)
            return count;

        for (int i = 0; i < m_nLength - 1; i++) {
            for (int j = 0; j < otherPath->length() - 1; j++) {
                if (GetVertex(i) == otherPath->GetVertex(j)
                        && GetVertex(i + 1) == otherPath->GetVertex(j + 1)) {
                    count++;
                    continue;
                }
            }
    ...
    bool SubPath(std::vector<BaseVertex*>& sub_path,
            BaseVertex* ending_vertex_pt) {

        for (std::vector<BaseVertex*>::const_iterator pos =
                m_vtVertexList.begin(); pos != m_vtVertexList.end(); ++pos) {
...
// display the content
    void PrintOut(std::ostream& out_stream) const {
        out_stream << "Cost: " << m_dWeight << " Length: "
                << m_vtVertexList.size() << std::endl;
        for (std::vector<BaseVertex*>::const_iterator pos =
                m_vtVertexList.begin(); pos != m_vtVertexList.end(); ++pos) {
