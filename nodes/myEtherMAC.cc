...
// TODO: there is some code that is pretty much the same as the one found in myEtherMACFullDuplex.cc (e.g. myEtherMAC::beginSendFrames)
...
static std::ostream&
operator<<(std::ostream& out, cMessage *msg)
{
  out << "(" << msg->getClassName() << ")" << msg->getFullName();
  return out;
}

Define_Module(myEtherMAC);

simsignal_t myEtherMAC::collisionSignal = SIMSIGNAL_NULL;
simsignal_t myEtherMAC::backoffSignal = SIMSIGNAL_NULL;
...
void
myEtherMAC::initializeStatistics()
{
...
void
myEtherMAC::processConnectDisconnect()
{
  if (!connected)
  {
    delete frameBeingReceived;
...
void
myEtherMAC::readChannelParameters(bool errorWhenAsymmetric)
{
...
void
myEtherMAC::handleSelfMessage(cMessage *msg)
{
...
void
myEtherMAC::handleMessage(cMessage *msg)
{
  if (dataratesDiffer)
...
void
myEtherMAC::processFrameFromUpperLayer(EtherFrame *frame)
{
  ...
  {
    error("Logic error: frame %s from higher layer has local MAC address as dest (%s)", frame->getFullName(), frame->getDest().str().c_str());
  }

  if (frame->getByteLength() > MAX_ETHERNET_FRAME_BYTES)
  {
    error("Packet from higher layer (%d bytes) exceeds maximum Ethernet frame size (%d)", (int) (frame->getByteLength()), MAX_ETHERNET_FRAME_BYTES);
  }
  ...
  else
  {
    ...
    {
      //2013-11-23 21:29:34 delete frame is queue is full
      EV << "txQueue length exceeds " << txQueue.innerQueue->getQueueLimit()
            << " -- this is probably due to a bogus app model generating excessive traffic (or if this is normal, increase txQueueLimit!)";
      delete frame;
      return;
      // error("txQueue length exceeds %d -- this is probably due to a bogus app model generating excessive traffic (or if this is normal, increase txQueueLimit!)", txQueue.innerQueue->getQueueLimit());
    }
...
void
myEtherMAC::addReceptionInReconnectState(long packetTreeId, simtime_t endRxTime)
{
...
void
myEtherMAC::addReception(simtime_t endRxTime)
{
...
void
myEtherMAC::processReceivedJam(EtherJam *jam)
{
...
void
myEtherMAC::processMsgFromNetwork(EtherTraffic *msg)
{
  ...
  if (!duplexMode)
  {
    ...
    if (propagationTime >= curEtherDescr->maxPropagationDelay)
    {
      error("Very long frame propagation time detected, maybe cable exceeds "
          "maximum allowed length? (%lgs corresponds to an approx. %lgm cable)", SIMTIME_STR(propagationTime), SIMTIME_STR(propagationTime * SPEED_OF_LIGHT_IN_CABLE));
    }
  }
  ...
  else if (receiveState == RECEIVING_STATE && !jamMsg && endRxMsg->getArrivalTime() - simTime() < curEtherDescr->halfBitTime)
  {
...
void
myEtherMAC::processDetectedCollision()
{
...
void
myEtherMAC::startFrameTransmission()
{
...
void
myEtherMAC::scheduleEndRxPeriod(EtherTraffic *frame)
{
...
void
myEtherMAC::handleEndBackoffPeriod()
{
...
void
myEtherMAC::handleEndJammingPeriod()
{
...
void
myEtherMAC::handleRetransmission()
{
...
void
myEtherMAC::handleEndPausePeriod()
{
...
void
myEtherMAC::frameReceptionComplete()
{
...
void
myEtherMAC::processReceivedDataFrame(EtherFrame *frame)
{
...
void
myEtherMAC::processReceivedPauseFrame(EtherPauseFrame *frame)
{
...
void
myEtherMAC::scheduleEndIFGPeriod()
{
  ASSERT(curTxFrame);

  if (frameBursting && (simTime() == lastTxFinishTime) && (framesSentInBurst > 0) && (framesSentInBurst < curEtherDescr->maxFramesInBurst)
...
void
myEtherMAC::scheduleEndTxPeriod(EtherFrame *frame)
{
...
void
myEtherMAC::scheduleEndPausePeriod(int pauseUnits)
{
