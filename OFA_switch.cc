    ...
    if (msg->isSelfMessage()) {
        if (msg->getKind() == MSGKIND_FLOW_ENTRY_TIMER) {
            oxm_basic_match *match =
                    (oxm_basic_match *) msg->getContextPointer();
...
void OFA_switch::connect() {
    ...
    if (getParentModule()->getParentModule()->getSubmodule("controller")
            != NULL) {
        // multiple controllers; full path is needed for connect address
        connectAddress = getParentModule()->getParentModule()->getSubmodule(
                "controller")->getFullPath().c_str();
    }
...
void OFA_switch::handlePacketOutMessage(Open_Flow_Message *of_msg) {
    ...
    if (buffer_id != OFP_NO_BUFFER) {
        ...
        EthernetIIFrame *frame =
                dynamic_cast<EthernetIIFrame *>(packet_out_msg->decapsulate());
...
// packet stored in buffer
void OFA_switch::execute_packet_out_action(ofp_action_header *action,
        uint32_t buffer_id, uint32_t inport) {
...
void OFA_switch::execute_packet_out_action(ofp_action_header *action,
        EthernetIIFrame *frame, uint32_t inport) {
...
void OFA_switch::handleFlowModMessage(Open_Flow_Message *of_msg) {
    ...
    uint8_t mod_com = flow_mod_msg->getCommand();
    oxm_basic_match *match = &flow_mod_msg->getMatch();

    if (mod_com == OFPFC_DELETE) {
        flow_table->deleteEntry(match);
    }
