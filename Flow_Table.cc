...
static inline int flow_fields_match(oxm_basic_match *m1, oxm_basic_match *m2,
        uint32_t w) {

    return (((w & OFPFW_IN_PORT) || m1->OFB_IN_PORT == m2->OFB_IN_PORT)
            && ((w & OFPFW_DL_TYPE) || m1->OFB_ETH_TYPE == m2->OFB_ETH_TYPE)
            && ((w & OFPFW_DL_SRC)
                    || !m1->OFB_ETH_SRC.compareTo(m2->OFB_ETH_SRC))
            && ((w & OFPFW_DL_DST)
                    || !m1->OFB_ETH_DST.compareTo(m2->OFB_ETH_DST))
            && ((w & OFPFW_NW_PROTO)
                    || (m1->OFB_IPV4_SRC == m2->OFB_IPV4_SRC
                            && m1->OFB_IPV4_DST == m2->OFB_IPV4_DST))
            && ((w & OFPFW_TP_SRC) || m1->OFB_TP_SRC == m2->OFB_TP_SRC)
            && ((w & OFPFW_TP_DST) || m1->OFB_TP_DST == m2->OFB_TP_DST));
}
...
void Flow_Table::addEntry(oxm_basic_match *match, entry_data* data) {
    ...
    while (j != entry_map.end()) {
        ...
        EV << j->first.OFB_IPV4_SRC << endl;
        EV << j->first.OFB_IPV4_DST << endl;
        EV << j->first.OFB_TP_SRC << endl;
        EV << j->first.OFB_TP_DST << endl;
        EV << j->first.wildcards << endl;
        j++;
    }

    EV << "Number of entries in " << this->getFullName() << ":"
            << entry_map.size() << endl;
    size = entry_map.size();
}
